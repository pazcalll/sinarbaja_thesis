<div class="container nowrap" style="width: 100%; right: 0px; background-color: white;" id="main-content">
    <h2>List Ready To Send Order</h2>
...
<script>
    function sendOrder(status_pembayaran, id) {
        if (status_pembayaran == "BELUM DIBAYAR") {
            toastr["error"]('Customer belum bayar')
        }
    }
    $(document).ready(function() {
        ...
        function format ( d ) {
            ...
            `
            if (d.status_pembayaran == "DISETUJUI SEMUA") {
                closerTbl += `
                    <button type="button" onclick="sendOrder('${d.status_pembayaran}', ${d.id})" class="btn btn-primary" style="width:100%">SEND</button>
                `
            }
            return closerTbl;
        }
